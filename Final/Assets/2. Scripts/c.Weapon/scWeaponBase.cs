...
public abstract class scWeaponBase : MonoBehaviour, IWeaponStats
{
    ...
    [HideInInspector]public scPlayerStat pS;
    ...
    public float attackSpeed { get{if (pS == null){Debug.LogWarning($"[{gameObject.name}] 플레이어 스탯을 찾을 수 없습니다. 기본 속도적용."); return baseAttackSpeed; }
            return baseAttackSpeed + pS.weaponAttackSpeed;}}   //공격 속도
    public int criticalDmg { get{if (pS == null) { Debug.LogWarning($"[{gameObject.name}] 플레이어 스탯을 찾을 수 없습니다. 기본 치뎀적용."); return baseCritDmg; }
                return baseCritDmg+(pS.weaponCritDmgUpgradeCnt);}}      //치명타 데미지
    public int criticalProb { get{if (pS == null) { Debug.LogWarning($"[{gameObject.name}] 플레이어 스탯을 찾을 수 없습니다. 기본 치확적용."); return baseCritProb; }
