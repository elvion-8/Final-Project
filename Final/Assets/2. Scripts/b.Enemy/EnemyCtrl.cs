...
public class EnemyCtrl : MonoBehaviour, ITakeDamage
{
    ...
    void UpdateModeState()
    {
        ...
        if (dist <= attackDist)
            ChangeState(hasPlayedAggro ? MODE_STATE.ATTACK : MODE_STATE.SURPRISE);
    ...
    IEnumerator AttackCoroutine()
    {
        ...
        Debug.Log($"[EnemyCtrl] 공격 트리거 = {prefab.name}");
        _anim.SetTrigger(prefab.name);
