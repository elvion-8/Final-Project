    ...
    public partial class FTailAnimator2_Editor
    {
            ...
            if (drawCollisions && Get.UseCollision)
            {
                ...
                if (Get.CollisionMode == TailAnimator2.ECollisionMode.m_3DCollision)
                {
                    ...
                    if (Get.CollisionSpace == TailAnimator2.ECollisionSpace.World_Slow)
                    {
                        ...
                        if( EditorGUI.EndChangeCheck())
                        { GetSelectedTailAnimators(); for (int i = 0; i < lastSelected.Count; i++) { EditorUtility.SetDirty( lastSelected[i] ); lastSelected[i].CollidersType = Get.CollidersType; new SerializedObject(lastSelected[i]).ApplyModifiedProperties(); } }
                        ...
                        if (EditorGUI.EndChangeCheck())
                        { GetSelectedTailAnimators(); for (int i = 0; i < lastSelected.Count; i++) { EditorUtility.SetDirty( lastSelected[i] ); lastSelected[i].CollideWithOtherTails = Get.CollideWithOtherTails; new SerializedObject(lastSelected[i]).ApplyModifiedProperties(); } }
        ...
        void Fold_ModuleIK()
        {
            ...
            if (drawIK && Get.UseIK)
            {
                ...
                if (Get.IKTarget == null && lastBoneKnown)
                {
                    ...
                    if (EditorGUI.EndChangeCheck()) { serializedObject.ApplyModifiedProperties(); EditorUtility.SetDirty( Get ); }
                    GUI.color = c;
                }
                else
                ...
                if (Get.UseWind && drawWindSettings)
                {
                    ...
                    if (Application.isPlaying == false)
                    {
                        if (TailAnimatorWind.Instance == null)
                        {
                            ...
                            {
                                TailAnimatorWind wind = FUVH.FindSceneObject<TailAnimatorWind>();
        ...
        void Fold_ModuleMaxDistance()
        {
            ...
            if (drawMaxDistance && Get.UseMaxDistance)
            {
                ...
                if (Get.DistanceFrom == null)
                {
                    ...
                    {
                        Camera c = FUVH.FindSceneObject<Camera>();
                        if (c) t = c.transform;
                    }
                    ...
                    GUI.color = c;
                    if (EditorGUI.EndChangeCheck()) { Get.DistanceFrom = tt; serializedObject.ApplyModifiedProperties(); EditorUtility.SetDirty( Get ); }
                }
        ...
        void Fold_TweakingAdditional()
        {
            ...
            if (drawTweakAdditional)
            {
                ...
                if (EditorGUI.EndChangeCheck()) { GetSelectedTailAnimators(); for (int i = 0; i < lastSelected.Count; i++) { EditorUtility.SetDirty( lastSelected[i] ); lastSelected[i].Axis2D = Get.Axis2D; new SerializedObject(lastSelected[i]).ApplyModifiedProperties(); } }
